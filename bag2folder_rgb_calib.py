...
class OfflineStereoRectifier:
    def __init__(self):
        ...
        # Baseline T_1_0
        x, y, z, w = -0.00280387, 0.64675888, -0.27093027,  0.71294592
        self.R_10 = np.array([
            [1 - 2*(y**2 + z**2), 2*(x*y - z*w),     2*(x*z + y*w)],
            [2*(x*y + z*w),       1 - 2*(x**2 + z**2), 2*(y*z - x*w)],
            [2*(x*z - y*w),       2*(y*z + x*w),     1 - 2*(x**2 + y**2)]
        ])
        self.t_10 = np.array([-0.04980, -0.01895, -0.04538])
...
def extract_and_rectify(bag_path, output_dir):
    ...
    with rosbag.Bag(bag_path, 'r') as bag, \
        ...
        for topic, msg, t in bag.read_messages(topics=[TOPIC_CAM0, TOPIC_CAM1, TOPIC_IMU]):
            ...
            if topic == TOPIC_CAM0:
                # [修改点] 将 "mono8" 改为 "bgr8" 以提取彩色图像
                cv_img = bridge.imgmsg_to_cv2(msg, "bgr8")
                # 核心：直接在这里使用预计算好的映射表进行插值校正 (remap 天然支持 3 通道)
